...
import { CreateUserSettingsCommand } from '../../application';
...
export class UserSettingsConsumer implements OnModuleInit {
  ...
  public async onModuleInit(): Promise<void> {
    this.consumer = this.kafka.consumer({ groupId: 'user-settings-group' });
    ...
    await this.consumer.run({
      ...
      eachMessage: async (message: EachMessagePayload) => {
        ...
        const result = await this.commandBus.execute(
          new CreateUserSettingsCommand(new UserIdVO(event.id)),
        );

        if (result.isOk()) {
          this.logger.log(`Created user settings for user ${event.id}`);
        }

        if (result.isErr()) {
          this.logger.error(
            `Error creating user settings for user ${event.id}: ${result.error.message}`,
          );
          return;
        }
