...
export class LoginUserCommandHandler
  ...
  async execute(command: LoginUserCommand) {
    ...
    const isPasswordValid = await this.cryptoPort.comparePassword(command.password, user.passwordHash);
    ...
    const sessionExpirationDate = new Date(currentDate.getTime() + SESSION_EXPIRATION_TIME);
